import { eachDayOfInterval, add, startOfDay, format, parse, eachMinuteOfInterval, isBefore, areIntervalsOverlapping, sub } from "date-fns";
import { TZDate } from "@date-fns/tz";

function getDateSlots (meetings, pref, duration) {
  const present = TZDate.tz(pref.timezone);
  ...
  datesMonthFiltered.forEach(date => {
    ...
    let possibleStarts = eachMinuteOfInterval({start: dayStart, end: endStart}, {step: pref.step}).filter(date => isBefore(add(date, {minutes: duration}), endStart));
    ...
    meetings.forEach(meeting => {
      possibleStarts = possibleStarts.filter(date => {
        return !areIntervalsOverlapping({start: date, end: add(date, {minutes: duration})}, {start: add(meeting.startTime, { minutes: -pref.gap }), end: add(meeting.endTime, { minutes: pref.gap })});
      });
    });
